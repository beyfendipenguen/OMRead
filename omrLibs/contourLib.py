...
def getRectangleContours(imgBinary):
    ...
    imgContours = cv2.findContours(
        imgBinary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def getBubbleContours(imgBinary):
    ...
    for c in imgContours:  # Konturları geziyoruz.
        ...
        # Burada konturun ait olduğu cismin köşe koordinatlarını buluyoruz.
        (x, y, w, h) = cv2.boundingRect(c)
        # Bulunan koordinatlardan yararlanarak cismin en boy oranını buluyoruz.
        ...
        if w >= 20 and h >= 20 and aspectRatio >= 0.9 and aspectRatio <= 1.1:
            ...
            # Dörtgenleri es geçip kabarcık konturlarını ekliyoruz.
